...
void RegisterState::handleInput(const InputManager& input) {
    (void)input;
    ...
    if (IsKeyPressed(KEY_ENTER)) {
        if (saveManager.registerAccount(inputUsername, inputPassword)) {
            // Success! Pop the register state, and push the login state
            GameEngine::getInstance().getStateManager().popState();
            GameEngine::getInstance().getStateManager().pushState(new LoginState());
        } else {
            errorMessage = "Username already exists or empty!";
        }
    }
}

void RegisterState::update(float dt) {
    (void)dt;
}

void RegisterState::drawTextBox(const std::string& label, const std::string& text, int x, int y, bool active, bool isPassword) {
...
void RegisterState::draw() {
    ...
    DrawText("REGISTER PROFILE", 250, 160, 25, MAROON);
